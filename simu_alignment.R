...
### heterosckdasticity & mean shift & scaling


sig.v=(seq(1,8, length.out= 20)) 
time=matrix(ncol=9,nrow=100)
error12=matrix(ncol=9,nrow=100)
error12.out=matrix(ncol=9,nrow=length(sig.v))
error2=matrix(ncol=9,nrow=100)
error2.out=matrix(ncol=9,nrow=length(sig.v))
error1=matrix(ncol=9,nrow=100)
error1.out=matrix(ncol=9,nrow=length(sig.v))
n=600
theta=n^(4/5)

#set.seed(35)
#A=matrix(rnorm(r^2),nrow=r)
A1=  diag(c(0.2,5,0.2)) #diag(c(5,0.2,0.2)) #A%*% t(A)
A2= solve(A1)
...
for(pp in 1:length(sig.v)){
  ...
  for(NN in 1:100){
    ...
    data.Y1 = data.Z
    data.Y1[,1:r] = data.Y1[,1:r]+as.matrix(data.X1*theta) %*% A1*.2
    data.Y1[,1] = data.Y1[,1] + theta*sig.v[pp]

    
    data.Z=matrix(rnorm(n*p,0,sig1), ncol=p)
    data.Y2 = data.Z
    data.Y2[,1:r] = data.Y2[,1:r]+as.matrix(data.X2*theta)%*% A2
...
for(pp in 1:length(sig.v)){
  ...
  for(NN in 1:100){
    ...
    data.Z=matrix(rnorm(n*p,0,sig1), ncol=p)
    data.Y1 = data.Z
    data.Y1[,1:r] = data.Y1[,1:r]+as.matrix(data.X1*theta) %*% A1
    data.Y1[,1] = data.Y1[,1] + theta
    ...
    data.Y2[,(r+1):(p)] = data.Y2[,(r+1):(p)]+matrix(runif((n)*length((r+1):(p)),theta/5,theta/4),ncol=length((r+1):(p)))
